...
class ConfigWYF:
    """
    ConfigWYF class for handling config operations
    """
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"ConfigWYF initialized successfully")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize ConfigWYF: {e}")
            return False
        ...
        for source in config_sources:
            try:
                if isinstance(source, str) and source.endswith('.json'):
                    ...
                    env_vars = {k: v for k, v in source.items() if k.startswith('CONFIG_')}
...
def create_config_instance(config: Optional[Dict] = None) -> ConfigWYF:
    """Factory function to create config instance"""
    instance = ConfigWYF(config)
    instance.initialize()
    return instance


def validate_config_config(config: Dict) -> bool:
