...
class ApiCVY:
    """
    ApiCVY class for handling api operations
    """
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"ApiCVY initialized successfully")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize ApiCVY: {e}")
            return False
        ...
        for source in config_sources:
            try:
                if isinstance(source, str) and source.endswith('.json'):
                    ...
                    env_vars = {k: v for k, v in source.items() if k.startswith('API_')}
...
def create_api_instance(config: Optional[Dict] = None) -> ApiCVY:
    """Factory function to create api instance"""
    instance = ApiCVY(config)
    instance.initialize()
    return instance
