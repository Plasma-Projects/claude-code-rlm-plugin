...
class ServicesFYE:
    """
    ServicesFYE class for handling services operations
    """
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"ServicesFYE initialized successfully")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize ServicesFYE: {e}")
            return False
        ...
        for source in config_sources:
            try:
                if isinstance(source, str) and source.endswith('.json'):
                    ...
                    env_vars = {k: v for k, v in source.items() if k.startswith('SERVICES_')}
...
def create_services_instance(config: Optional[Dict] = None) -> ServicesFYE:
    """Factory function to create services instance"""
    instance = ServicesFYE(config)
    instance.initialize()
    return instance


def validate_services_config(config: Dict) -> bool:
