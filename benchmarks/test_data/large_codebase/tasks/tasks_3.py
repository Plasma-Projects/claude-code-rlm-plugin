...
class TasksQFT:
    """
    TasksQFT class for handling tasks operations
    """
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"TasksQFT initialized successfully")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize TasksQFT: {e}")
            return False
        ...
        for source in config_sources:
            try:
                if isinstance(source, str) and source.endswith('.json'):
                    ...
                    env_vars = {k: v for k, v in source.items() if k.startswith('TASKS_')}
...
def create_tasks_instance(config: Optional[Dict] = None) -> TasksQFT:
    """Factory function to create tasks instance"""
    instance = TasksQFT(config)
    instance.initialize()
    return instance


def validate_tasks_config(config: Dict) -> bool:
