...
class DatabaseVTT:
    """
    DatabaseVTT class for handling database operations
    """
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"DatabaseVTT initialized successfully")
            return True
        except Exception as e:
            logger.error(f"Failed to initialize DatabaseVTT: {e}")
            return False
        ...
        for source in config_sources:
            try:
                if isinstance(source, str) and source.endswith('.json'):
                    ...
                    env_vars = {k: v for k, v in source.items() if k.startswith('DATABASE_')}
...
def create_database_instance(config: Optional[Dict] = None) -> DatabaseVTT:
    """Factory function to create database instance"""
    instance = DatabaseVTT(config)
    instance.initialize()
    return instance
